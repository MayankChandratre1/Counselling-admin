...
class UserListService {
     ...
     */
    async assignListToUser(userId, listId, admin, overrideTitle = null) {
        try {
            ...
            const userList = new UserList({
                ...
                type: 'assigned',
                title: overrideTitle || masterList.title,
                colleges: masterList.colleges || [],
                isCustomized: false,
     ...
     */
    async updateUserList(userListId, updateData, admin) {
        try {
     ...
     */
    async updateCreatedUserList(userListId, updateData, admin) {
        try {
            const updated = await UserList.findOneAndUpdate(
                { id: userListId, type: 'created' },
                { $set: { ...updateData, lastUpdatedBy: admin.email } },
                { new: true }
            );
    ...
    async releaseListFromUser(userId, userListId, admin) {
        try {
            const userList = await UserList.findOne({ id: userListId, userId });
            if (!userList) throw new Error('User list not found');

            const originalListId = userList.originalListId;

            await UserList.deleteOne({ id: userListId });
    ...
    async deleteUserList(userListId, admin) {
        try {
            const userList = await UserList.findOne({ id: userListId });
            if (!userList) throw new Error('User list not found');
    ...
    async deleteUserCreatedList(userListId, admin) {
        try {
            const userList = await UserList.findOne({ id: userListId, type: 'created' });
